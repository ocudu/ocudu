...
static void fill_cu_up_xnu_socket_entry(YAML::Node& node, const xnu_socket_appconfig& config)
{
...
static void fill_xnu_socket_section(YAML::Node& node, const std::vector<xnu_socket_appconfig>& sock_cfg)
{
  auto sock_node = node["socket"];
  for (const auto& cfg : sock_cfg) {
    YAML::Node node_sock;
    fill_cu_up_xnu_socket_entry(node_sock, cfg);
    sock_node.push_back(node_sock);
  }
}

void ocudu::fill_xnu_config_yaml_schema(YAML::Node& node, const xnu_sockets_appconfig& config)
{
  node["bind_port"] = config.bind_port;
  node["peer_port"] = config.peer_port;
  fill_xnu_socket_section(node, config.xnu_socket_cfg);
}
