...
public:
  ...
  error_type<std::string> is_valid(const ssb_processor::pdu_t& pdu, unsigned cell_bandwith_prbs) const override
  {
    ...
    // The SS/PBCH is expected to be transmitted over a single layer.
    if (pdu.precoding_and_beamforming.get_nof_layers() != 1) {
      return make_unexpected(fmt::format("The SS/PBCH is expected to be transmitted over a single layer, requested {}.",
