...
bwp_time_domain_mapper::bwp_time_domain_mapper(const bwp_time_domain_builder_params& params)
{
  ...
  if (const auto* auto_res = std::get_if<builder_params::auto_resources>(&params.params)) {
    ...
    // Derive k1 values.
    dedicated_k1_candidates = time_domain_resource_helper::generate_k1_candidates(params.tdd_cfg, auto_res->min_k1);
    common_min_k1           = auto_res->min_k1;
  } else {
    const auto& explicit_res = std::get<builder_params::explicit_resources>(params.params);
    ocudu_assert(not explicit_res.pdsch_td_res_list.empty() and not explicit_res.pusch_td_res_list.empty() and
                     not explicit_res.k1_candidates.empty(),
                 "Explicit PDSCH/PUSCH TD resource and k1 candidate lists must not be empty.");
    ...
    dedicated_k1_candidates.assign(explicit_res.k1_candidates.begin(), explicit_res.k1_candidates.end());
  }

  // Generate the PUSCH and PDSCH candidates for each slot (handles both FDD and TDD).
