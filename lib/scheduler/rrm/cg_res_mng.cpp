...
static circular_vector<crb_bitmap> build_alloc_grid(const ran_cell_config& cell_cfg)
{
  ...
  const unsigned bwp_nof_crbs = cell_cfg.ul_cfg_common.init_ul_bwp.generic_params.crbs.length();
  ...
  const std::vector<pucch_resource> cell_pucch_res_list = config_helpers::generate_cell_pucch_res_list(
      cell_cfg.init_bwp.pucch.resources, cell_cfg.ul_cfg_common.init_ul_bwp.generic_params.crbs.length());
  crb_bitmap pucch_crbs =
      compute_pucch_crbs(cell_cfg.ul_cfg_common.init_ul_bwp.generic_params.crbs,
...
std::optional<unsigned> cg_type1_res_mng::cell_context::find_optimal_cg_offset()
{
...
void cg_type1_res_mng::add_cell(du_cell_index_t cell_idx, const ran_cell_config& cell_cfg)
{
  ...
  // maximum number of RBs reserved for CG in the cell) is enforced by the scheduler cell configuration validator.
...
void cg_type1_res_mng::dealloc_resources(ue_cell_config& ue_cell_cfg)
{
  if (not cells.contains(ue_cell_cfg.serv_cell_cfg.cell_index)) {
    return;
  }
