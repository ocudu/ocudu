                                                            ...
                                                            unsigned                      max_rbs = MAX_NOF_PRBS)
{
  ...
  // PDSCH TD resource indices via the pooled dl_time_domain_mapper. Restricting the search to that same per-slot
  // index set avoids scanning candidates known not to apply to pdcch_slot.
  const dci_dl_format dci_format = ss.get_dl_dci_format();
  for (const uint8_t pdsch_td_index : bwp_cfg.dl.td_mapper().pdsch_td_res_indices(dci_format, pdcch_slot.count())) {
    const pdsch_time_domain_resource_allocation& pdsch_td_res =
        bwp_cfg.dl.td_mapper().pdsch_td_resources(dci_format)[pdsch_td_index];
