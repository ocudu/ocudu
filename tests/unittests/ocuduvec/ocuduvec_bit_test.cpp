...
using OcuduvecBitParams = unsigned;

class OcuduvecBitFixture : public ::testing::TestWithParam<OcuduvecBitParams>
{
protected:
  unsigned size;
  unsigned nbits;
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestUnpack)
{
  ...
  // Unpack
  span<uint8_t> bit_buf = ocuduvec::bit_unpack(unpacked, value, size);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestUnpackVector)
{
  ...
  // Unpack
  ocuduvec::bit_unpack(unpacked, packed);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestPack)
{
  ...
  unsigned            value   = ocuduvec::bit_pack(bit_buf, size);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestPackVector)
{
  ...
  // Unpack
  ocuduvec::bit_pack(packed, unpacked);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestPackFullVector)
{
  ...
  unsigned packed = ocuduvec::bit_pack(unpacked);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestPackOffsetVector)
{
  std::uniform_int_distribution<unsigned> dist(0, 1U);
  ...
  ocudu::ocuduvec::bit_pack(packed, offset, unpacked);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestCopyOffsetVector)
{
  ...
  for (auto read_offset : read_offsets) {
    ...
    ocuduvec::copy_offset(destination, source, read_offset);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestCopyOffsetBitBuffers)
{
  std::uniform_int_distribution<unsigned> dist(0, 1U);
  ...
  for (auto read_offset : read_offsets) {
    for (auto write_offset : write_offsets) {
      ...
      // Copy from source to destination.
      ocuduvec::copy_offset(destination, write_offset, source, read_offset, nbits);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestUnpackVectorWithRemainder)
{
  std::uniform_int_distribution<unsigned> dist(0, UINT8_MAX);
...
TEST_P(OcuduvecBitFixture, OcuduvecBitTestUnpackVectorOffset)
{
  ...
  // Unpack
  ocuduvec::bit_unpack(unpacked, packed, offset);
