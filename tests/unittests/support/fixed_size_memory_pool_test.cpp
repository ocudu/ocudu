...
TEST(fixed_memory_block_pool_test, correct_instantiation_hugepage)
{
  struct test_tag_id {};

  auto& pool = fixed_size_memory_block_pool<test_tag_id, true>::get_instance(128, 256, true);
  ASSERT_EQ(pool.nof_memory_blocks(), 128);
  ASSERT_EQ(pool.memory_block_size(), 256);
  pool.print_all_buffers();
}
...
TEST(fixed_memory_block_pool_test, allocated_block_is_valid_hugepage)
{
  struct test_tag_id {};
  auto& pool  = fixed_size_memory_block_pool<test_tag_id, true>::get_instance(128, 256, true);
  void* block = pool.allocate_node(256);
...
TEST(fixed_memory_block_pool_test, number_of_alloc_blocks_matches_pool_size_hugepage)
{
  struct test_tag_id {};
  auto& pool = fixed_size_memory_block_pool<test_tag_id, true>::get_instance(128, 256, true);
...
TEST(fixed_memory_block_pool_test, when_worker_local_cache_exceeds_threshold_returns_block_to_central_cache_hugepage)
{
  const unsigned nof_blocks = 128;
  struct test_tag_id {};
  auto&       pool = fixed_size_memory_block_pool<test_tag_id, true>::get_instance(nof_blocks, 256, true);
  task_worker w("WorkerDealloc", nof_blocks * 4);
